...
def analyse(input_string, syno_list):  # 输入字符串, 输出列表形式的最小项之和
    ...
    while len(new_string) > 0:
        ...
        if new_string[0] != "(":
            ...
            single_word = merge_syn(syno_list, [single_word])[0]  # 同近义词替换
...
def merge_syn(syno_list, words):
    res = []
    for word in words:
        flag = False
        for item in syno_list:
            if word in item:
                res.append(item[0])
                flag = True
                break
...
def input_processor(req_type):  # 获取用户输入的布尔表达式, 输出最小项的和. 如[['A', 'NOT B'], ['C', 'D']]表示A~B+CD
    # 对每个词都做近义词替代
    if req_type == 'movies':
        syno_file = '../../movies/doc/syno_dict_movies.json'
    else:
        syno_file = '../../books/doc/syno_dict_books.json'
    with open(syno_file, 'r', encoding='utf-8') as f:
        syno_list = json.load(f)
    ...
    user_input = input(">>")
    result = analyse(user_input.strip(), syno_list)
    # print(result)
    return result


# if __name__ == "__main__":
#     input_processor("books")
